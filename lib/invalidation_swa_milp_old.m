function [Decision,sol] = invalidation_swa_milp(SYS,input,output,mn_bnd,...
    input_bnd,state_bnd,solver)
...
if(isempty(input_bnd))
    input_bnd=zeros(n_i,1)+inf;
end
if(isempty(state_bnd))
    state_bnd=zeros(n,1)+inf;
end
if(isempty(solver))
    solver='cplex';
end
...
s = binvar(T,n_mode,'full');
e = sdpvar(n_mode*T,n_y,'full');
X = sdpvar(n_mode*T,2*n,'full');
