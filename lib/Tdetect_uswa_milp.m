...
d_A = binvar(4,n_mode,nf_mode,T);
d_B = binvar(4,n_mode,nf_mode,T);
d_C = binvar(4,n_mode,nf_mode,T);
d_D = binvar(4,n_mode,nf_mode,T);
fd_A = binvar(4,n_mode,nf_mode,T);
fd_B = binvar(4,n_mode,nf_mode,T);
fd_C = binvar(4,n_mode,nf_mode,T);
fd_D = binvar(4,n_mode,nf_mode,T);
% real variables
    ...
    for sys_ind = 1: n_mode  % system mode index
        for flt_ind = 1: nf_mode  % fault mode index
                ...
                for j= 1:n
                    ...
                    for q=1:4
                        Constraint = [Constraint, SA(i,j,q,sys_ind,flt_ind,t)<=...
                            d_A(q,sys_ind,flt_ind,t)*M_u(j)*abs(Mode(sys_ind).N_A(i,j)), ...
                            SA(i,j,q,sys_ind,flt_ind,t)>=...
                            d_A(q,sys_ind,flt_ind,t)*M_l(j)*abs(Mode(sys_ind).N_A(i,j)), ...
                            zc_A(j,q,sys_ind,flt_ind,t)<=...
                            d_A(q,sys_ind,flt_ind,t)*M_u(j), ...
                            zc_A(j,q,sys_ind,flt_ind,t)>=...
                            d_A(q,sys_ind,flt_ind,t)*M_l(j)];
                    end
                    Constraint = [Constraint, sum(SA(i,j,:,sys_ind,flt_ind,t)) == DA(i,j,sys_ind,flt_ind,t)];
                end
                Constraint = [Constraint, sum(d_A(:,sys_ind,flt_ind,t)) == d(sys_ind,flt_ind,t)];
                ...
                for j= 1:n_u
                    ...
                    for q=1:4
                        Constraint = [Constraint, SB(i,j,q,sys_ind,flt_ind,t)<=...
                            d_B(q,sys_ind,flt_ind,t)*abs(Mode(sys_ind).N_B(i,j))*U_u(j), ...
                            SB(i,j,q,sys_ind,flt_ind,t)>=...
                            d_B(q,sys_ind,flt_ind,t)*abs(Mode(sys_ind).N_B(i,j))*U_l(j), ...
                            tt_B(j,q,sys_ind,flt_ind,t)<=...
                            d_B(q,sys_ind,flt_ind,t)*U_u(j), ...
                            tt_B(j,q,sys_ind,flt_ind,t)>=...
                            d_B(q,sys_ind,flt_ind,t)*U_l(j)];
                    end
                    Constraint = [Constraint, sum(SB(i,j,:,sys_ind,flt_ind,t)) == DB(i,j,sys_ind,flt_ind,t)];
                    Constraint = [Constraint, sum(tt_B(j,:,sys_ind,flt_ind,t),2) == tt(j,sys_ind,flt_ind,t)];
                end
                Constraint = [Constraint, sum(d_B(:,sys_ind,flt_ind,t)) == d(sys_ind,flt_ind,t)];
            end
            % Uncertainty for C
            for i = 1:n_y
                for j= 1:n
                    ...
                    for q=1:4
                        Constraint = [Constraint, SC(i,j,q,sys_ind,flt_ind,t)<=...
                            d_C(q,sys_ind,flt_ind,t)*M_u(j)*abs(Mode(sys_ind).N_C(i,j)), ...
                            SC(i,j,q,sys_ind,flt_ind,t)>=...
                            d_C(q,sys_ind,flt_ind,t)*M_l(j)*abs(Mode(sys_ind).N_C(i,j)), ...
                            zc_C(j,q,sys_ind,flt_ind,t)<=...
                            d_C(q,sys_ind,flt_ind,t)*M_u(j), ...
                            zc_C(j,q,sys_ind,flt_ind,t)>=...
                            d_C(q,sys_ind,flt_ind,t)*M_l(j)];
                    end
                    Constraint = [Constraint, sum(SC(i,j,:,sys_ind,flt_ind,t)) == DC(i,j,sys_ind,flt_ind,t)];
                    Constraint = [Constraint, sum(zc_C(j,:,sys_ind,flt_ind,t)) == cz(j,sys_ind,flt_ind,t)];
                end
                Constraint = [Constraint, sum(d_C(:,sys_ind,flt_ind,t)) == d(sys_ind,flt_ind,t)];
            end
            % Uncertinty for D
            for i = 1:n_y
                for j= 1:n_u
                    ...
                    for q=1:4
                        Constraint = [Constraint, SD(i,j,q,sys_ind,flt_ind,t)<=...
                            d_D(q,sys_ind,flt_ind,t)*abs(Mode(sys_ind).N_D(i,j))*U_u(j), ...
                            SD(i,j,q,sys_ind,flt_ind,t)>=...
                            d_D(q,sys_ind,flt_ind,t)*abs(Mode(sys_ind).N_D(i,j))*U_l(j), ...
                            tt_D(j,q,sys_ind,flt_ind,t)<=...
                            d_D(q,sys_ind,flt_ind,t)*U_u(j), ...
                            tt_D(j,q,sys_ind,flt_ind,t)>=...
                            d_D(q,sys_ind,flt_ind,t)*U_l(j)];
                    end
                ...
                Constraint = [Constraint, sum(d_D(:,sys_ind,flt_ind,t)) == d(sys_ind,flt_ind,t)];  
            end
            ...
            for i = 1:n
                for j= 1:n
                    ...
                    for q=1:4
                        Constraint = [Constraint, fSA(i,j,q,sys_ind,flt_ind,t)<=...
                            fd_A(q,sys_ind,flt_ind,t)*Mf_u(j)*abs(fMode(flt_ind).N_A(i,j)), ...
                            fSA(i,j,q,sys_ind,flt_ind,t)>=...
                            fd_A(q,sys_ind,flt_ind,t)*Mf_l(j)*abs(fMode(flt_ind).N_A(i,j)), ...
                            fzc_A(j,q,sys_ind,flt_ind,t)<=...
                            fd_A(q,sys_ind,flt_ind,t)*Mf_u(j), ...
                            fzc_A(j,q,sys_ind,flt_ind,t)>=...
                            fd_A(q,sys_ind,flt_ind,t)*Mf_l(j)];
                    end
                ...
                Constraint = [Constraint, sum(fd_A(:,sys_ind,flt_ind,t)) == d(sys_ind,flt_ind,t)];
            end
            % Uncertainty in B
            for i = 1:n
                for j= 1:n_u
                    ...
                    for q=1:4
                        Constraint = [Constraint, fSB(i,j,q,sys_ind,flt_ind,t)<=...
                            fd_B(q,sys_ind,flt_ind,t)*U_u(j), ...
                            fSB(i,j,q,sys_ind,flt_ind,t)>=...
                            fd_B(q,sys_ind,flt_ind,t)*abs(fMode(flt_ind).N_B(i,j))*U_l(j), ...
                            ftt_B(j,q,sys_ind,flt_ind,t)<=...
                            fd_B(q,sys_ind,flt_ind,t)*abs(fMode(flt_ind).N_B(i,j))*U_u(j), ...
                            ftt_B(j,q,sys_ind,flt_ind,t)>=...
                            fd_B(q,sys_ind,flt_ind,t)*U_l(j)];
                    end
                    Constraint = [Constraint, sum(fSB(i,j,:,sys_ind,flt_ind,t)) == fDB(i,j,sys_ind,flt_ind,t)];
                    Constraint = [Constraint, sum(ftt_B(j,:,sys_ind,flt_ind,t)) == tt(j,sys_ind,flt_ind,t)];
                end
                Constraint = [Constraint, sum(fd_B(:,sys_ind,flt_ind,t)) == d(sys_ind,flt_ind,t)];
            end
            % Uncertainty for C
            for i = 1:n_y
                for j= 1:n
                    ...
                    for q=1:4
                        Constraint = [Constraint, fSC(i,j,q,sys_ind,flt_ind,t)<=...
                            fd_C(q,sys_ind,flt_ind,t)*Mf_u(j)*abs(fMode(flt_ind).N_C(i,j)), ...
                            fSC(i,j,q,sys_ind,flt_ind,t)>=...
                            fd_C(q,sys_ind,flt_ind,t)*Mf_l(j)*abs(fMode(flt_ind).N_C(i,j)), ...
                            fzc_C(j,q,sys_ind,flt_ind,t)<=...
                            fd_C(q,sys_ind,flt_ind,t)*Mf_u(j), ...
                            fzc_C(j,q,sys_ind,flt_ind,t)>=...
                            fd_C(q,sys_ind,flt_ind,t)*Mf_l(j)];
                    end
                    Constraint = [Constraint, sum(fSC(i,j,:,sys_ind,flt_ind,t)) == fDC(i,j,sys_ind,flt_ind,t)];
                    Constraint = [Constraint, sum(fzc_C(j,:,sys_ind,flt_ind,t)) == cz_f(j,sys_ind,flt_ind,t)];
                end
                Constraint = [Constraint, sum(fd_C(:,sys_ind,flt_ind,t)) == d(sys_ind,flt_ind,t)];
            end
            % Uncertinty for D
            for i = 1:n_y
                for j= 1:n_u
                    ...
                    for q=1:4
                        Constraint = [Constraint, fSD(i,j,q,sys_ind,flt_ind,t)<=...
                            fd_D(q,sys_ind,flt_ind,t)*abs(fMode(flt_ind).N_D(i,j))*U_u(j), ...
                            fSD(i,j,q,sys_ind,flt_ind,t)>=...
                            fd_D(q,sys_ind,flt_ind,t)*abs(fMode(flt_ind).N_D(i,j))*U_l(j), ...
                            ftt_D(j,q,sys_ind,flt_ind,t)<=...
                            fd_D(q,sys_ind,flt_ind,t)*U_u(j), ...
                            ftt_D(j,q,sys_ind,flt_ind,t)>=...
                            fd_D(q,sys_ind,flt_ind,t)*U_l(j)];
                    end
                ...
                Constraint = [Constraint, sum(fd_D(:,sys_ind,flt_ind,t)) == d(sys_ind,flt_ind,t)]; 
            end
        end
    end
end
